...
pFile = open("relat_nordeste_leve.txt", "w")
...
def mapeamento_genotipo_fenotipo(NENT, NSAI, aleatorio, TIPO, file):
    ...
    N_TIPO = np.random.random_integers(400, 500, 1)[0]

    NINT = N_TIPO
    NINT1 = N_TIPO
    NINT2 = np.random.random_integers(400, 500, 1)[0]
...
def treina_rede_(contind, file, NINT1, NINT2):
    ...
    model.add(Dense(units=NINT2, activation='relu'))
    ...
    model.fit(x_train, y_train, epochs=7000, batch_size=10)
    ...
    for i, predic in enumerate(rounded):
        ...
        pFile.write("\nPadrao>>"+str(i))
        pFile.write("\ncalculado>>" + str(predic) + "   \tdesejado>>" + str(y_test.iloc[i]) + "  \tErro>>"+str(err))
    ...
    plt.savefig('leve/i_'+str(contind)+'.png', dpi=None, facecolor='w', edgecolor='w',
